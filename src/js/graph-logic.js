(function () {
  ...
  function createCircularLayout(count) {
  ...
  function buildAdjacency(nodeCount, edges) {
    const adjacency = Array.from({ length: nodeCount }, function () {
      return Array(nodeCount).fill(false);
    });
  ...
  function buildGraph(nodeCount, edges, targetSize) {
    ...
    return {
      nodes: layout.map(function (node, index) {
        return {
          id: `v${index + 1}`,
          label: node.label,
          x: node.x,
  ...
  function buildNodeColorMap(nodes) {
    return Object.fromEntries(
      nodes.map(function (node, index) {
        return [node.id, NODE_COLORS[index % NODE_COLORS.length]];
      }),
    );
  }
  ...
  function createSampleCliqueGraph() {
    return buildGraph(
      5,
      [
        [0, 1],
  ...
  function createSampleIndependentSetGraph() {
    return buildGraph(
      6,
      [
        [0, 1],
  ...
  function createSampleVertexCoverGraph() {
    return buildGraph(
      5,
      [
        [0, 1],
  ...
  function addGraphNode(graph) {
    ...
    const adjacency = graph.adjacency.map(function (row) {
      return row.concat(false);
    });
  ...
  function updateGraphNodePosition(graph, nodeId, x, y) {
  ...
  function toggleGraphEdge(graph, fromIndex, toIndex) {
  ...
  function setGraphTarget(graph, targetSize, minimum) {
    return {
      ...cloneGraph(graph),
  ...
  function createEmptyGraph(nodeCount, targetSize) {
    return buildGraph(nodeCount, [], clampTargetSize(nodeCount, targetSize, 1));
  }

  function generateRandomCliqueGraph(nodeCount, density, targetSize) {
  ...
  function generateRandomIndependentSetGraph(nodeCount, density, targetSize) {
    const nextGraph = createEmptyGraph(nodeCount, targetSize);
    const plantedSet = new Set(
      shuffle(range(nodeCount)).slice(0, nextGraph.targetSize),
    );
  ...
  function generateRandomVertexCoverGraph(nodeCount, density, targetSize) {
    const nextGraph = createEmptyGraph(nodeCount, targetSize);
  ...
  function complementIndices(count, indices) {
    const selected = new Set(indices);
    return range(count).filter(function (index) {
      return !selected.has(index);
    });
  }

  function complementGraph(graph, targetSize) {
    const size = graph.nodes.length;
    const adjacency = Array.from({ length: size }, function () {
  ...
  function reduceIndependentSetToClique(graph) {
    return complementGraph(graph, graph.targetSize);
  }

  function reduceVertexCoverToClique(graph) {
    const cliqueTarget = clamp(
      graph.nodes.length - graph.targetSize,
      0,
      graph.nodes.length,
    );
  ...
  function sortCandidatesByDegree(graph, indices) {
    return indices.slice().sort(function (left, right) {
  ...
  function solveCliqueBruteForce(graph) {
    const initialCandidates = sortCandidatesByDegree(
      graph,
      ...
      for (let index = 0; index < candidates.length; index += 1) {
        ...
        const nextCandidates = candidates
          ...
          .filter(function (candidate) {
            return graph.adjacency[vertex][candidate];
          });
  ...
  function solveIndependentSetBruteForce(graph) {
    const cliqueResult = solveCliqueBruteForce(
      reduceIndependentSetToClique(graph),
    );
  ...
  function solveVertexCoverBruteForce(graph) {
    const cliqueResult = solveCliqueBruteForce(
      reduceVertexCoverToClique(graph),
    );
  ...
  function reduceCliqueToSat(graph) {
    const variableOrder = [];
    const variableMeta = {};
    const clauses = [];
    const familyCounts = {
    ...
    for (let slot = 0; slot < graph.targetSize; slot += 1) {
      clauses.push({
        ...
        literals: graph.nodes.map(function (node) {
          return {
  ...
  function decodeCliqueAssignment(assignment, reduction) {
    const selected = [];

    reduction.variableOrder.forEach(function (variable) {
  ...
  function describeNodeSelection(indices, nodes) {
