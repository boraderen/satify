(function () {
  ...
  const { formatAssignmentPreview } = window.SATifySat;

  function pageLink(rootPath, path) {
    return `${rootPath}${path || ""}`;
  }

  function homeLink(rootPath) {
  ...
  function renderBrand(rootPath) {
    return `
      <div class="masthead">
        <a class="brand-link" href="${homeLink(rootPath)}">
  ...
  function renderDefinition(copy) {
    return `
      <div class="definition-block">
        ${copy.definition
          .map(function (item) {
            return `<p><strong>${escapeHtml(item.label)}.</strong> ${escapeHtml(item.text)}</p>`;
          })
          .join("")}
  ...
  function renderHomePage(rootPath) {
    return `
        ...
        ${renderWindow(
          ...
          `
            <table class="problem-table">
              ...
              <tbody>
                ${SUPPORTED_PROBLEMS.map(function (problem) {
                  return `
                    <tr>
                      <td>
    ...
    return renderWindow(
      ...
      `
        ...
        <div class="formula-editor">
          ${state.draftClauses
            .map(function (clause, clauseIndex) {
              return `
                <article class="clause-editor-card">
                  <div class="clause-editor-head">
                    <strong>C${clauseIndex + 1}</strong>
                    <button data-action="formula-remove-clause" data-clause-id="${escapeHtml(clause.id)}" ${state.draftClauses.length <= 1 || state.busyAction ? "disabled" : ""}>Remove</button>
                  </div>
                  <div class="literal-row">
                    <span>(</span>
                    ${clause.literals
                      .map(function (literal, literalIndex) {
                        return `
                          <div class="literal-editor">
                            ...
                            <select data-action="formula-select-variable" data-clause-id="${escapeHtml(clause.id)}" data-literal-index="${literalIndex}" ${state.busyAction ? "disabled" : ""}>
                              ${variableNames
                  ...
                  ${
                    !isThreeSat
                      ? `
                        <div class="toolbar-row">
                          <button data-action="formula-add-literal" data-clause-id="${escapeHtml(clause.id)}" ${clause.literals.length >= state.config.maxClauseLength || state.busyAction ? "disabled" : ""}>Add literal</button>
                        </div>
                      `
                      : ""
                  }
                      ...
                      slotCount
                        ? `
                          <div class="legend-values">
                            ${Array.from({ length: slotCount }, function (_, index) {
                              return printableVariable(`x_${index + 1}_${node.label}`);
                            })
                              .map(function (variable) {
                                return `<span class="token-chip">${escapeHtml(variable)}</span>`;
                              })
                              .join("")}
                          </div>
                        `
            ...
            ${activeReduction.inputFormula.clauses
              .map(function (clause) {
                const mappedClauses = activeReduction.threeSat.clauses.filter(function (candidate) {
                  return (candidate.sourceClauseId ?? candidate.id) === clause.id;
                });
  ...
  function renderProblemPage(options) {
    const editor =
      options.state.kind === "graph"
        ? renderGraphEditor(options.state)
        : renderFormulaEditor(options.state);
    const results =
      options.state.kind === "graph"
        ? renderGraphResults(options.state)
        : renderFormulaResults(options.state);
